...
export class Portfolio extends React.Component {
    ...
    createButton() {
        if (this.state.currentShow[1]===5) {
            return <button onClick={() => this.setState({currentShow: [0,8]})} className='Portfolio-button'><FormattedMessage id='Portfolio-button1'/></button>
        }
        else if (this.state.currentShow[1]===8) {
            return <button onClick={() => this.setState({currentShow: [0,12]})} className='Portfolio-button'><FormattedMessage id='Portfolio-button2'/></button>
        }
        else {return <a href="https://github.com/galdranorn" target="_blank"><button className='Portfolio-button'><FormattedMessage id='Portfolio-button3'/></button></a>}
    }
